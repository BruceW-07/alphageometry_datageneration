...
def shuffle_in_batch(texts, shuffle_type):
    if shuffle_type=='sentence':
        return shuffle_sentences_in_batch(texts)
    elif shuffle_type=='svg_content':
        return shuffle_svg_content(texts)
    elif shuffle_type == 'none':
        return texts
    else:
        raise ValueError(f'Shuffle type {shuffle_type} not understood')

def shuffle_svg_content(texts):
    """shuffles every line except for the first two lines and the last line to maintain SVG tags"""
    result = []
    for text in texts:
        # Split each text into sentences
        content_lines = text.split('\n')
        # Shuffle the sentences
        shufflable_content_lines = content_lines[2:-1]
        random.shuffle(shufflable_content_lines)
        # Join the shuffled sentences back into a single string
        result.append('\n'.join(content_lines[:2] + shufflable_content_lines + content_lines[-1:]))
...
if __name__ == "__main__":
    org_cnt = '<svgl1>\n<svgl2>\nlc1\nlc2\nlc3\n<svg_end_line>'
    print(org_cnt)
    shuffled = shuffle_svg_content([org_cnt,])
    print(shuffled[0])
#     import re
