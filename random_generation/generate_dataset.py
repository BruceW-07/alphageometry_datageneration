...
def run(pid, search_depth, samples_per_thread):
    random.seed(pid)
    ...
    filename = (f'./dataset/geometry_depth{search_depth}_{pid}.csv')
...
if __name__ == "__main__":
    ...
    if args.n_threads == 1:
        run(args.run_id, args.search_depth, args.samples_per_thread)
    else:
        with multiprocessing.Pool(args.n_threads) as pool:
            pool.starmap(run, [(args.run_id * args.n_threads + i, args.search_depth, args.samples_per_thread)
                                for i in range(args.n_threads)])
    end = time.process_time()
    logger.info(f'Generate {args.n_threads * args.samples_per_thread} in {end - start} seconds.')